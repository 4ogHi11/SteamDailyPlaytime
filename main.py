...
def get_playing_time():
    ...
    # 获取当天的日期，以获取准确的文件名
    today_date = datetime.datetime.now(pytz.timezone("Asia/Shanghai")).strftime("%Y%m%d")
    # 获取昨天的日期
    yesterday_date = (
    ...
    昨天_game_info = pd.read_csv(f"./data/steam_data/steam_data_{yesterday_date}.csv")
    ...
    merge_game_info["playtime_date"] = (
        datetime.datetime.当前(pytz.timezone("Asia/Shanghai")) - datetime.timedelta(1)
    ).date()
